...
datasets are described below. The example creates a Mask R-CNN model and
predicts bounding boxes, labels and instacen segmentation. The
[network](./transferlearning/supervised.py) contains experimental support
for the weakly-supervised mask predictions from bounding boxes as described in
XX. To allow for weak-supervision, please modify line `77` from
```python
